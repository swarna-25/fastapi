...
def test_get_all_posts(authorized_client,test_posts):
    ...
    assert res.status_code==201 #changed this from 200
...
@pytest.mark.parametrize("title,content,published",[
    ("created title at first","created content at first",True),
    ("this is some new title","this is sone new content",False),
    ("Pizza is the new title","i like farmhouse pizza",True)
])
...
def test_create_post_default_published_true(authorized_client,test_user,test_posts):
    res= authorized_client.post("/posts/",json={"title":"random title","content":"some content"})
    created_post=schemas.Post(**res.json())
    assert res.status_code==201
...
def test_unauthorized_user_create_post(client,test_posts,test_user):
    res= client.post("/posts/",json={"title":"a new title","content":"thi is new content"})
    assert res.status_code==401
...
def test_update_other_user_post(authorized_client,test_posts,test_user,test_user2):
    data ={
        "title":"updating other user post",
        "content":"updating other user content",
        "id": test_posts[3].id
    }
...
def test_update_post_non_exist(authorized_client,test_posts,test_user):
    ...
    res= authorized_client.put(f"/posts/879098",json=data)
    assert res.status_code==404
